...
export default function EditJournalScreen() {
  ...
  const { journals, updateJournal, deleteJournal } = useDatabase();
  ...
  const [imagePath, setImagePath] = useState<string | null>(entry?.imagePath ?? null);
  ...
  const pickImage = async () => {
    ...
    if (!res.granted) { Alert.alert("Permission needed", "Allow photo access."); return; }
  ...
  const takePhoto = async () => {
    ...
    if (!res.granted) { Alert.alert("Permission needed", "Allow camera access."); return; }
    const result = await ImagePicker.launchCameraAsync({ quality: 0.85, allowsEditing: true });
    if (!result.canceled) setImagePath(result.assets[0].uri);
  };

  const handleSave = async () => {
    if (!content.trim()) { Alert.alert("Empty", "Write something first."); return; }
    setSaving(true);
    try {
      await updateJournal(id, { content: content.trim(), mood, imagePath: imagePath ?? undefined });
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      >
        <View style={styles.topRow}>
          ...
          <TouchableOpacity onPress={handleDelete} style={[styles.deleteBtn, { backgroundColor: colors.destructive + "18" }]}>
            <Feather name="trash-2" size={16} color={colors.destructive} />
          </TouchableOpacity>
        </View>

        <View style={[styles.moodCard, { backgroundColor: colors.card }]}>
          <Text style={[styles.moodLabel, { color: colors.text }]}>How were you feeling?</Text>
        ...
        <View style={[styles.photoSection, { backgroundColor: colors.card }]}>
          ...
          {imagePath ? (
            <View>
              ...
              <TouchableOpacity style={[styles.removeBtn, { backgroundColor: colors.destructive }]} onPress={() => setImagePath(null)}>
            ...
            <View style={styles.photoButtons}>
              <TouchableOpacity style={[styles.photoBtn, { backgroundColor: colors.secondary }]} onPress={pickImage}>
                <Feather name="image" size={18} color={colors.primary} />
                <Text style={[styles.photoBtnTxt, { color: colors.primary }]}>Gallery</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.photoBtn, { backgroundColor: colors.secondary }]} onPress={takePhoto}>
